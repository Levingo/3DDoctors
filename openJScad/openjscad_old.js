var includeFileList = []; // for including array of files.
var includeFontJS = " ";  // for including one font file

function includeFontOpenJScad(s) {  // s = font file name  // for including one font file
	includeFontJS = s;
}

function openJScadInclude(s) { // for including list of files - call it only 1 time on OnLoad
	includeFileList.push(s);
}
...
OpenJsCad.Viewer = function(containerelement, width, height, initialdepth) {
  ...
  gl.clearColor(0.73, 0.8, 1, 1); //CLR background
...
OpenJsCad.parseJsCadScriptASync = function(script, mainParameters, options, callback) {
  ...
  for(var iii=0;iii<includeFileList.length;iii++) {
 	  baselibraries.push("../../openJScad/"+includeFileList[iii]);
  }

  if (includeFontJS.length>2) {
	  baselibraries.push("../../openJScad/"+includeFontJS);
  }
