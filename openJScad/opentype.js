...
(function () {
    ...
    Path.prototype.draw = function (ctx,narrowing,italictan) {
        var i, cmd;
        ctx.beginPath();
        for (i = 0; i < this.commands.length; i += 1) {
            cmd = this.commands[i];
            if (cmd.type === 'M') {
	            cmd.x=10+(cmd.x+(25-cmd.y)*italictan)*narrowing;
	            ...
	            cmd.x=10+(cmd.x+(25-cmd.y)*italictan)*narrowing;
	            cmd.x1=10+(cmd.x1+(25-cmd.y1)*italictan)*narrowing;
	            cmd.x2=10+(cmd.x2+(25-cmd.y2)*italictan)*narrowing;
    ...
    // OFER FUNCTION TO CREATE CAG ********************************************************************
    Path.prototype.createCAG = function (narrowing, italictan) {
        var i, cmd;
        //ctx.beginPath();
        var textCAG = new CAG();
        ...
        var curX=0;
        var curY=0;
        var flip=1; //set -1 for flip union and subtract
        var firstpath=true;
        ...
        for (i = 0; i < this.commands.length; i += 1) {
            ...
            if (cmd.type === 'M') {
                //ctx.moveTo(cmd.x, cmd.y);
                Y=-cmd.y;
                X=(cmd.x+Y*italictan)*narrowing;
                
                if (inpath==true) {  // need to close and open
                    PATH=PATH.close();
                    sumdir+=(X0-curX)*(curY+Y0);
                    if ((firstpath==true)&&(sumdir<0)) { flip=-1; }
                    if (sumdir*flip<0) {   // PATH is clockwise
	                    textCAG=textCAG.subtract(PATH.innerToCAG());
                    }
                    else  {
	                    textCAG=textCAG.union(PATH.innerToCAG());
                    }
                    ii+=1;
                    firstpath=false;
                    sumdir=0;
    			...
    			var PATH = new CSG.Path2D([[curX,curY]]);
                ...
                X=(cmd.x+Y*italictan)*narrowing;
                Y1=-cmd.y1;
                X1=(cmd.x1+Y1*italictan)*narrowing;
                Y2=-cmd.y2;
                X2=(cmd.x2+Y2*italictan)*narrowing;
                
                PATH=PATH.appendBezier([[X1,Y1], [X2,Y2], [X,Y]]);
                sumdir+=(X1-curX)*(Y+curY);
                sumdir+=(X2-X1)*(Y2+Y1);
                sumdir+=(X-X2)*(Y+Y2);
                curX=X;
                curY=Y;
                ...
                PATH=PATH.appendBezier([[X1,Y1], [X,Y]]);
                sumdir+=(X1-curX)*(Y1+curY);
                sumdir+=(X-X1)*(Y+Y);
                curX=X;
                curY=Y;
                inpath=true;
    ...
    // Options is an optional object that contains:
    // - kerning - Whether to take kerning information into account. (default: true)
    Font.prototype.draw = function (ctx, text, x, y, fontSize, options, narrowing, italictan) {
        this.getPath(text, x, y, fontSize, options).draw(ctx, narrowing, italictan);
    };

    Font.prototype.getCAG = function (text, x, y, fontSize, options, narrowing, italictan) {
        return this.getPath(text, x, y, fontSize, options).createCAG(narrowing, italictan);
    };
